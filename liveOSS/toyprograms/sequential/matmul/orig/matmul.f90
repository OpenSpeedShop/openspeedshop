...
!
  ...
  integer, parameter :: order_max = 100
...
!
!    Input, integer NREP, the number of repetitions to carry out.
!
...
!
  ...
  a(1:n,1:n) = cmplx ( 0.0E+00, 0.0E+00 )
  b(1:n,1:n) = cmplx ( 2.0E+00, 1.0E+00 )
  c(1:n,1:n) = cmplx ( 1.0E+00, 1.0E+00 )
...
!
  ...
  else if ( s_eqi ( order, 'I46_IJK' ) ) then

    call i46_ijk ( lda, n, nrep, ttime )

  else if ( s_eqi ( order, 'I4_MATMUL' ) ) then

    call i4_matmul ( lda, n, nrep, ttime )

  else if ( s_eqi ( order, 'L_IJK' ) ) then

    call l_ijk ( lda, n, nrep, ttime )
  ...
  else if ( s_eqi ( order, 'R4_SGEMMS' ) ) then

    call r4_sgemms ( lda, n, nrep, ttime )
...
!    to be shown.
!
!    Output, integer NREP, the number of repetitions to carry out.
!
!    Output, logical NREP_SHOW, is TRUE if the number of repetitions is
!    to be shown.
!
...
! machine = 'VECTOR/VAX'
  ...
  if ( s_eqi ( machine(1:4), 'CRAY') ) then
    order_num = order_num + 1
    order_list(order_num) = 'I46_IJK' 
  end if

  order_num = order_num + 1
  order_list(order_num) = 'L_IJK'
  ...
  if ( s_eqi ( machine(1:4), 'CRAY') ) then
    order_num = order_num + 1
    order_list(order_num) = 'R4_SGEMMS'
  end if
...
subroutine i46_ijk ( lda, n, nrep, ttime )
!DIR$integer = 46
!
!  The above line is a Cray compiler directive, which requests that
...
!  Discussion:
!
!    On the Cray, it is possible to request that integer arithmetic
!    be done in 46 bit mode, even though the integer is stored as
!    a 64 bit quantity.  The arithmetic is generally done much faster 
!    if it is known in advance that only 46 bits are involved.
!
!  Modified:
!
!    28 August 1999
...
subroutine l_ijk ( lda, n, nrep, ttime )
...
!
!    Input/output, integer LDA, the leading dimension used for arrays.
...
!
  ...
  real b(lda,n)
  real c(lda,n)
  integer, parameter :: inca = 1
  integer, parameter :: incb = 1
  integer, parameter :: incc = 1
  integer irep
  integer nrep
  real time1
  real time2
...
subroutine r4_sgemms ( lda, n, nrep, ttime )
...
!   call sgemms ( 'n', 'n', n, n, n, alpha, b, lda, c, lda, beta, a, lda, work )
...
!  Parameters:
!
!    Input, character ( len = 6 ) SRNAME, the routine which called TERBLA.
!
