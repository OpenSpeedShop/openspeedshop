...
# set $OPENSSROOT_PREFIX using the shell-based conditional then %define prefix to what
# we set $OPENSSROOT_PREFIX to be.
if [ $OPENSSROOT_PREFIX ]; then
	echo "- \$OPENSSROOT_PREFIX is defined: setting prefix macro to $OPENSSROOT_PREFIX"
else
	set $OPENSSROOT_PREFIX=%{prefix}
	echo "- \$OPENSSROOT_PREFIX is undefined: prefix macro is %{prefix}"
fi
%define prefix %(echo $OPENSSROOT_PREFIX)

if [ $OPENSSROOT_DEPEND ]; then
	echo "- \$OPENSSROOT_DEPEND is defined: setting prefix macro to $OPENSSROOT_PREFIX"
else
	set OPENSSROOT_DEPEND=%{prefix}
	echo "- \$OPENSSROOT_DEPEND is undefined: prefix macro is %{prefix}"
fi
%define depend_prefix %(echo $OPENSSROOT_DEPEND)
