...
ggBoolean mrPhongAreaXYRectangleLuminaire::shadowHit(        
             const ggRay3& ray,
             double time,
             double tmin,
             double tmax,
   ...
   if (mrXYRectangle::shadowHit(ray,time,tmin,tmax,t,N,emits,E)) {
       emits = ggTrue;
       double cost = -ray.direction().z() * N.z() / ray.direction().length();
...
ggBoolean mrPhongAreaXYRectangleLuminaire::viewingHit(        
             const ggRay3& ray,
             double time,
             double tmin,
             double tmax,
             ...
             ) const
{
   ...
   if (mrXYRectangle::viewingHit(ray, time, tmin, tmax, VHR, MR)) {
       VHR.hasEmit = ggTrue;
       double cost = -ray.direction().z() * VHR.UVW.w().z() /
...
ggBoolean mrPhongAreaXYRectangleLuminaire::selectVisiblePoint(
...
ggBoolean mrPhongAreaXYRectangleLuminaire::approximateDirectRadiance(
