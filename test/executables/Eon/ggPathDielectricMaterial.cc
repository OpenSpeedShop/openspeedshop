...
#include <ggPathDielectricMaterial.h>
...
ggPathDielectricMaterial::ggPathDielectricMaterial(const ggSpectrum& c1,
                                           const double& c2)
{
    #ifdef GGSAFE
        assert(c1[0] >= 1.0);
...
ggBoolean ggPathDielectricMaterial::getInfo(const ggRay3& r_in,
                                        const ggPoint3& p,
                                        const ggONB3& uvw,  
                                        const double& ,  
    ...
    if (ggDot(n, v_in) >= 0.0) { // ray is in material  
        ...
        if (ggDielectricVacuumRefract(an, Nt[0], refracted_v)) {
	       ...
	       mat.UV.Set(t1, ggCanonicalRandom());
             ...
             if (ggCanonicalRandom() < Rs)
                 mat.ray1.Set(p, rv);
             else
                 mat.ray1.Set(p, refracted_v);
             mat.kRay1.Set(1.0);
        }
