...
ggBoolean mrDiffuseAreaYZRectangleLuminaire::shadowHit(        
             const ggRay3& ray,
             double time ,
             double tmin,
             double tmax,
...
ggBoolean mrDiffuseAreaYZRectangleLuminaire::viewingHit(        
             const ggRay3& ray,
             double time,
             double tmin,
             double tmax,
...
ggBoolean mrDiffuseAreaYZRectangleLuminaire::selectVisiblePoint(
...
ggBoolean mrDiffuseAreaYZRectangleLuminaire::approximateDirectRadiance(
             ...
             const
{
      ...
      if ((cosl = ggDot(Nprime , tox)) > 0.00001 &&
          (cosx = -ggDot(N,tox)) > 0.00001)
           Lc =  (cosx*cosl*area) / ggSquaredDistance(p,centroid);

      tox = ggUnitVector(p - p0);
      if ((cosl = ggDot(Nprime , tox)) > 0.00001 &&
          (cosx = -ggDot(N,tox)) > 0.00001)
           L0 =  (cosx*cosl*area) / ggSquaredDistance(p,p0);

      tox = ggUnitVector(p - p1);
      if ((cosl = ggDot(Nprime , tox)) > 0.00001 &&
          (cosx = -ggDot(N,tox)) > 0.00001)
           L1 =  (cosx*cosl*area) / ggSquaredDistance(p,p1);

      tox = ggUnitVector(p - p2);
      if ((cosl = ggDot(Nprime , tox)) > 0.00001 &&
          (cosx = -ggDot(N,tox)) > 0.00001)
           L2 =  (cosx*cosl*area) / ggSquaredDistance(p,p2);

      tox = ggUnitVector(p - p3);
      if ((cosl = ggDot(Nprime , tox)) > 0.00001 &&
          (cosx = -ggDot(N,tox)) > 0.00001)
           L3 =  (cosx*cosl*area) / ggSquaredDistance(p,p3);
