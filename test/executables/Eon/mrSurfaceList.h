...
#ifndef MRSURFACELIST_H
#define MRSURFACELIST_H
...
public:
  ...
  virtual ggBoolean boundingBox(
   ...
   mrSurface * operator[] (int i) const { return surfaces[i]; }

   void Add(mrSurface* sPtr) { surfaces.Append(sPtr); }
   ggBoolean empty() const { return length() == 0 ; }

  protected:
    ggTrain<mrSurface *> surfaces;
};
