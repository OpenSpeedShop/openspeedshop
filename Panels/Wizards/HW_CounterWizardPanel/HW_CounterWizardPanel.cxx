 ...
 */
HW_CounterWizardPanel::HW_CounterWizardPanel(PanelContainer *pc, const char *n, void *argument) : Panel(pc, n)
{
  ...
  hwCounterFormLayout = new QVBoxLayout( sv->viewport(), 1, 2, getName() );
...
// End: add the bottom portion: The "wizard mode" and "brought to you by"
  ...
  hwCounterFormLayout->addLayout( bottomLayout );
  languageChange();
...
void HW_CounterWizardPanel::eAttachOrLoadPageClearButtonSelected()
{
...
void HW_CounterWizardPanel::vAttachOrLoadPageClearButtonSelected()
{
...
void HW_CounterWizardPanel::vAttachOrLoadPageAttachToProcessCheckBoxSelected()
{
...
void HW_CounterWizardPanel::eAttachOrLoadPageAttachToProcessCheckBoxSelected()
{
  if( eAttachOrLoadPageAttachToProcessCheckBox->isChecked() )
  {
...
void HW_CounterWizardPanel::vAttachOrLoadPageLoadExecutableCheckBoxSelected()
{
...
void HW_CounterWizardPanel::vAttachOrLoadPageLoadDifferentExecutableCheckBoxSelected()
{
...
void HW_CounterWizardPanel::eAttachOrLoadPageLoadExecutableCheckBoxSelected()
{
  if( eAttachOrLoadPageLoadExecutableCheckBox->isChecked() )
  {
    eAttachOrLoadPageLoadDifferentExecutableCheckBox->setChecked(FALSE);
...
void HW_CounterWizardPanel::eAttachOrLoadPageLoadDifferentExecutableCheckBoxSelected()
{
...
void HW_CounterWizardPanel::vAttachOrLoadPageNextButtonSelected()
{
  ...
  { 
    ...
    sprintf(buffer, "<p align=\"left\">You've selected a HW Counter experiment for process \"%s\" running on host \"%s\".  Futher you've chosed a sampling rate of \"%s\".<br><br>To complete the experiment setup select the \"Finish\" button.<br><br>After selecting the \"Finish\" button an experiment \"hwCounter\" panel will be raised to allow you to futher control the experiment.<br><br>Press the \"Back\" button to go back to the previous page.</p>", mw->pidStr.ascii(), "localhost", vParameterPageSampleRateText->text().ascii() );
  }
  if( vAttachOrLoadPageLoadExecutableCheckBox->isChecked() ||
      vAttachOrLoadPageLoadDifferentExecutableCheckBox->isChecked() )
  {
    ...
    sprintf(buffer, "<p align=\"left\">You've selected a HW Counter experiment for executable \"%s\" to be run on host \"%s\".  Futher you've chosed a sampling rate of \"%s\".<br><br>To complete the experiment setup select the \"Finish\" button.<br><br>After selecting the \"Finish\" button an experiment \"hwCounter\" panel will be raised to allow you to futher control the experiment.<br><br>Press the \"Back\" button to go back to the previous page.</p>", mw->executableName.ascii(), "localhost", vParameterPageSampleRateText->text().ascii() );
  }
...
//        printf("Warning: No attach or load paramaters available.\n");
      ...
      if( lao != NULL )
      {
        if( !p )
        {
          ...
          p = getPanelContainer()->getMasterPC()->dl_create_and_add_panel("HW Counter", getPanelContainer(), (void *)argument);
        }
...
"You've selected a HW Counter experiment for executable \"%s\" to be run on host \"%s\".  Futher you've chosed a sample rate of \"%d\".<br><br></p>" ) );
