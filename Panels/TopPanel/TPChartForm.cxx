...
*/
TPChartForm::TPChartForm( TopPanel *t5p, QSplitter *splitter, const char *n, int flags )
    : ChartForm( splitter, n, flags )
{
  topFivePanel = t5p;
  nprintf(DEBUG_CONST_DESTRUCT) ( "TPChartForm::TPChartForm( ) constructor called\n");
}

/*! The default destructor. */
TPChartForm::~TPChartForm( )
{
  // default destructor.
  nprintf(DEBUG_CONST_DESTRUCT) ("  TPChartForm::~TPChartForm( ) destructor called\n");
}

int
TPChartForm::mouseClicked( int item )
{
  nprintf(DEBUG_PANELS) ("TPChartForm::mouseClicked(item=%d) called.\n", item);
  topFivePanel->itemSelected(item);
}

void
TPChartForm::contentsContextMenuEvent( QContextMenuEvent *e)
{
...
QPopupMenu*
TPChartForm::createPopupMenu( const QPoint & pos )
{
  nprintf(DEBUG_PANELS) ("TPChartForm: Hello from down under the hood.\n");
