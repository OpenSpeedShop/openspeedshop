 ...
 */
MPIPanel::MPIPanel(PanelContainer *pc, const char *n, void *argument) : Panel(pc, n)
{
...
//      Thread t1 = *ti; 
  ...
  if( expID == -1 )
  {
    ...
    {
      command = QString("expCreate -f \"%1 %2\" mpi\n").arg(executableNameStr).arg(argsStr);
    } else if( !pidStr.isEmpty() )
    { 
      command = QString("expCreate %1 mpi\n").arg(pidStr);
    } else
    {
...
//  fprintf(stdout, "A: MY VALUE! = (%d)\n", val);
  ...
  {
    nprintf( DEBUG_PANELS ) ("mpi has been requested to open an existing experiment!!!\n");
...
topPC->dl_create_and_add_panel("MPI Wizard", bestFitPC, (char *)this);
    } else
    {
...
void
MPIPanel::manageCollectorsAndProcessesSelected()
{
  nprintf( DEBUG_PANELS ) ("MPIPanel::manageCollectorsAndProcessesSelected()\n");
...
// printf("MPIPanel::listener(%s) requested.\n", mo->msgType );
  ...
  {
//    printf("MPIPanel::listener() ClosingDownObject!\n");
