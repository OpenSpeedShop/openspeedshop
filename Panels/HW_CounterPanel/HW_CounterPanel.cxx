...
HW_CounterPanel::HW_CounterPanel(PanelContainer *pc, const char *n, void *argument) : Panel(pc, n)
{
  ...
  QWidget *HW_CounterControlPanelContainerWidget = new QWidget( getBaseWidgetFrame(),
                                        "HW_CounterControlPanelContainerWidget" );
  topPC = createPanelContainer( HW_CounterControlPanelContainerWidget,
                              "PCSamplingControlPanel_topPC", NULL,
                              pc->getMasterPCList() );
  frameLayout->addWidget( HW_CounterControlPanelContainerWidget );
...
printf("# theApplication.attachCollector(theCollector.getValue());\n");
  ...
  HW_CounterControlPanelContainerWidget->show();
  topPC->show();
  topLevel = TRUE;
  topPC->topLevel = TRUE;
...
void
HW_CounterPanel::loadNewProgramSelected()
{
  nprintf( DEBUG_PANELS ) ("HW_CounterPanel::loadNewProgramSelected()\n");
...
bool
HW_CounterPanel::detachFromProgramSelected()
{
  nprintf( DEBUG_PANELS ) ("HW_CounterPanel::detachFromProgramSelected()\n");
...
void
HW_CounterPanel::attachToExecutableSelected()
{
  nprintf( DEBUG_PANELS ) ("HW_CounterPanel::attachToExecutableSelected()\n");
  if( runnableFLAG == TRUE )
  {
    if( detachFromProgramSelected() == FALSE )
    {
...
{
  nprintf( DEBUG_PANELS ) ("HW_CounterPanel::manageCollectorsSelected()\n");
}   

void
HW_CounterPanel::manageProcessesSelected()
{
  nprintf( DEBUG_PANELS ) ("HW_CounterPanel::managerProcessesSelected()\n");
}   

void
HW_CounterPanel::manageDataSetsSelected()
{
  nprintf( DEBUG_PANELS ) ("HW_CounterPanel::managerDataSetsSelected()\n");
}   
...
HW_CounterPanel::saveAsSelected()
{
  ...
  if( sfd->exec() == QDialog::Accepted )
  {
    ...
    { 
      ...
      *sao->ts << "<title>HW_CounterReport</title>";
      *sao->ts << "<h2>HW_CounterReport</h2>";
      *sao->ts << "</head>";
