...
#   Change that (the default case entry) when you change the default vers to something other than 6.0
	...
	*)
    ...
    LDFLAGS="$DYNINST_LDFLAGS $BINUTILS_LDFLAGS $LIBDWARF_LDFLAGS $LIBELF_LDFLAGS $BOOST_LDFLAGS"
    LIBS="$CXXFLAGS $DYNINST_LIBS $BINUTILS_IBERTY_LIB $LIBDWARF_LIBS $LIBELF_LIBS"
