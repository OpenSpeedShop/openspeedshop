...
AC_DEFUN([AX_SYMTABAPI], [
	...
	*)
    ...
    LDFLAGS="$CXXFLAGS $SYMTABAPI_LDFLAGS $SYMTABAPI_LIBS $BINUTILS_LDFLAGS -liberty $LIBDWARF_LDFLAGS $LIBDWARF_LIBS $LIBELF_LDFLAGS $LIBELF_LIBS"

    AC_MSG_CHECKING([for symtabAPI API library and headers])

    LIBS="${LIBS} $SYMTABAPI_LDFLAGS $SYMTABAPI_LIBS $BINUTILS_LDFLAGS -liberty $LIBDWARF_LDFLAGS $LIBDWARF_LIBS $LIBELF_LDFLAGS $LIBELF_LIBS" 
