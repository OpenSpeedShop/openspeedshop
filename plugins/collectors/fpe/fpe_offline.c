...
/** @file
 *
 * Declaration and definition of the FPE collector's runtime (offline support).
 *
 */
...
#if 0
    fpe_start_sampling_args args;
    memset(&args, 0, sizeof(args));
    args.collector=1;
    args.sampling_rate=100000;
    args.thread=getpid();
    /* how to handle
       Blob arguments(reinterpret_cast<xdrproc_t>(xdr_fpe_start_sampling_args), &args);
    */
#endif
...
void offline_stop_sampling(const char* arguments)
{
     ...
     * blobs to the specific openss-raw file.
     */
    fpe_stop_tracing(arguments);
}
