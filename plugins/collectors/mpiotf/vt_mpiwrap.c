...
int mpiotf_PMPI_Finalize()
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Finalize called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Comm_dup( MPI_Comm comm,
                  MPI_Comm* newcomm )
{
  int result;
  uint64_t time;
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Comm_dup called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Comm_create( MPI_Comm comm,
                     MPI_Group group,
                     MPI_Comm* newcomm )
{
  int result;
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Comm_create called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Comm_split( MPI_Comm comm,
                    int color,
                    int key,
                    MPI_Comm* newcomm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Comm_split called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Cart_create( MPI_Comm comm_old,
                     int ndims,
                     int* dims,
                     int* periodv,
                     int reorder,
                     MPI_Comm* comm_cart)
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Cart_create called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Cart_sub ( MPI_Comm comm,
                   int *rem_dims,
                   MPI_Comm *newcomm)
{
  int result;
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Cart_sub called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Graph_create( MPI_Comm comm_old,
                      int nnodes,
                      int* index,
                      int* edges,
                      int reorder,
                      MPI_Comm* comm_graph)
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Graph_create called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Intercomm_create (MPI_Comm local_comm,
                          int local_leader,
                          MPI_Comm peer_comm,
                          int remote_leader,
                          int tag,
...

{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Intercomm_create called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Intercomm_merge (MPI_Comm intercomm,
                         int high,
                         MPI_Comm *newcomm)
{
  int result;
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Intercomm_merge called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Comm_free( MPI_Comm* comm )
{
  int result;
  uint64_t time;

  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Comm_free called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
	      ...
	      MPI_Comm comm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Send called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Bsend( void* buf,
	      int count,
	      MPI_Datatype datatype,
	      int dest,
	      int tag,
	      MPI_Comm comm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Bsend called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Rsend( void* buf,
               int count,
               MPI_Datatype datatype,
               int dest,
               int tag,
               MPI_Comm comm)
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Rsend called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Ssend( void* buf,
	      int count,
	      MPI_Datatype datatype,
	      int dest,
	      int tag,
	      MPI_Comm comm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Ssend called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
	      ...
	      MPI_Status* status )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Recv called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Probe( int source,
               int tag,
               MPI_Comm comm,
               MPI_Status* status )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Probe called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Sendrecv(void* sendbuf,
		 int sendcount,
		 ...
		 MPI_Status* status )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Sendrecv called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Sendrecv_replace(void* buf,
		         int count,
		         MPI_Datatype datatype,
		         int dest,
		         int sendtag,
		         ...
		         MPI_Status* status )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Sendrecv_replace called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Isend( void* buf,
	       int count,
	       MPI_Datatype datatype,
	       int dest,
	       int tag,
	       ...
	       MPI_Request* request )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Isend called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Irecv( void* buf,
	       int count,
	       MPI_Datatype datatype,
	       int source,
	       int tag,
...
int mpiotf_PMPI_Ibsend( void* buf,
                int count,
                MPI_Datatype datatype,
                int dest,
                int tag,
                ...
                MPI_Request *request)
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Ibsend called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Issend( void* buf,
                int count,
                MPI_Datatype datatype,
                int dest,
                int tag,
                ...
                MPI_Request *request)
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Issend called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Irsend( void* buf,
                int count,
                MPI_Datatype datatype,
                int dest,
                int tag,
                ...
                MPI_Request *request)
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Irsend called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Wait( MPI_Request* request,
	      MPI_Status* status)
{
  int result;
  MPI_Status mystatus;
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Wait called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Waitall( int count,
		 ...
		 MPI_Status* array_of_statuses )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Waitall called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Waitany( int count,
		 MPI_Request* requests,
		 int* index,
		 MPI_Status* status )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Waitany called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Waitsome(int incount,
		 ...
		 MPI_Status *array_of_statuses )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Waitsome called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Test( MPI_Request* request,
	      int* flag,
	      MPI_Status* status )
{
  int result;
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Test called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Testany( int count,
                 MPI_Request *array_of_requests,
		 int *index,
		 int *flag,
		 MPI_Status *status )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Testany called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Testall( int count,
                 MPI_Request *array_of_requests,
		 int *flag,
		MPI_Status *array_of_statuses )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Testall called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Testsome(int incount,
		 MPI_Request *array_of_requests,
		 int *outcount,
		 int *array_of_indices,
		 MPI_Status *array_of_statuses )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Testsome called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Send_init( void* buf,
		   int count,
		   MPI_Datatype datatype,
		   int dest,
		   int tag,
		   ...
		   MPI_Request* request )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Send_init called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Recv_init( void* buf,
		   int count,
		   MPI_Datatype datatype,
		   int source,
		   int tag,
		   ...
		   MPI_Request* request )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Recv_init called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Bsend_init( void* buf,
		   int count,
		   MPI_Datatype datatype,
		   int dest,
		   int tag,
		   ...
		   MPI_Request* request )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Bsend_init called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Ssend_init( void* buf,
		   int count,
		   MPI_Datatype datatype,
		   int dest,
		   int tag,
		   ...
		   MPI_Request* request )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Ssend_init called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Rsend_init( void* buf,
		   int count,
		   MPI_Datatype datatype,
		   int dest,
		   int tag,
		   ...
		   MPI_Request* request )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Rsend_init called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Start( MPI_Request* request )
{
  int result;
  uint64_t time;

  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Start called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Startall( int count,
                  MPI_Request *array_of_requests )
{
  int result, i;
  uint64_t time;
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Startall called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Request_free( MPI_Request* request )
{
  int result;
  uint64_t time;

  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Request_free called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Cancel( MPI_Request* request )
{
  int result;
  uint64_t time;

  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Cancel called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Allreduce ( void* sendbuf,
		    void* recvbuf,
		    int count,
		    MPI_Datatype datatype,
		    MPI_Op op,
		    MPI_Comm comm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Allreduce called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Barrier( MPI_Comm comm )
{
  int result;
  uint64_t time, etime;

  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Barrier called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Bcast( void* buf,
	       int count,
	       MPI_Datatype datatype,
	       int root,
	       MPI_Comm comm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Bcast called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Gather(void* sendbuf,
	       int sendcount,
	       MPI_Datatype sendtype,
	       void* recvbuf,
	       int recvcount,
	       ...
	       MPI_Comm comm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Gather called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Reduce( void* sendbuf,
		void* recvbuf,
		int count,
		MPI_Datatype datatype,
		MPI_Op op,
		...
		MPI_Comm comm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Reduce called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Gatherv( void* sendbuf,
		 int sendcount,
		 MPI_Datatype sendtype,
		 void* recvbuf,
		 int *recvcounts,
		 ...
		 MPI_Comm comm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Gatherv called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Allgather( void* sendbuf,
		   int sendcount,
		   MPI_Datatype sendtype,
		   void* recvbuf,
		   int recvcount,
		   ...
		   MPI_Comm comm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Allgather called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Allgatherv( void* sendbuf,
                    int sendcount,
                    MPI_Datatype sendtype,
                    void* recvbuf,
                    int *recvcounts,
                    ...
                    MPI_Comm comm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Allgatherv called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Alltoall( void* sendbuf,
                  int sendcount,
                  MPI_Datatype sendtype,
                  void* recvbuf,
                  int recvcount,
                  ...
                  MPI_Comm comm)
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Alltoall called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Alltoallv( void* sendbuf,
                   int *sendcounts,
                   int *sdispls,
                   MPI_Datatype sendtype,
                   void* recvbuf,
                   ...
                   MPI_Comm comm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Alltoallv called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Scan( void* sendbuf,
	      void* recvbuf,
              int count,
              MPI_Datatype datatype,
              MPI_Op op,
              MPI_Comm comm  )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Scan called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Scatter( void* sendbuf,
                 int sendcount,
                 MPI_Datatype sendtype,
                 void* recvbuf,
                 int recvcount,
                 ...
                 MPI_Comm comm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Scatter called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Scatterv( void* sendbuf,
                  int *sendcounts,
                  int *displs,
                  MPI_Datatype sendtype,
                  void* recvbuf,
                  ...
                  MPI_Comm comm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Scatterv called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
...
int mpiotf_PMPI_Reduce_scatter( void* sendbuf,
			void* recvbuf,
			int *recvcounts,
			MPI_Datatype datatype,
			MPI_Op op,
			MPI_Comm comm )
{
  ...
  if (debug_trace) {
    fprintf(stderr, "WRAPPER, mpiotf_PMPI_Reduce_scatter called, IS_TRACE_ON = %d \n", IS_TRACE_ON);
    fflush(stderr);
  }
