...
/** @file
 *
 * Declaration and definition of the IO sampling collector's runtime
 * (offline support).
 *
 */
...
#if 0
    io_start_sampling_args args;
    memset(&args, 0, sizeof(args));
    args.collector=1;
    args.sampling_rate=100000;
    args.thread=getpid();
    /* how to handle
       Blob arguments(reinterpret_cast<xdrproc_t>(xdr_io_start_sampling_args), &args);
    */
#endif
...
void offline_stop_sampling(const char* arguments)
{
    ...
    /* call the collector specific stop sampling funcion. This writes any */
    /* data blobs to the specific openss-raw file */
    /* (exp-collector-pid-tid.openss-raw). */
    io_stop_tracing(arguments);
