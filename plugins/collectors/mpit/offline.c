...
static const uint32_t TLSKey = 0x0000FEFA;
...
void offline_pause_sampling()
{
    ...
    if (getenv("OPENSS_DEBUG_COLLECTOR") != NULL) {
        fprintf(stderr,"offline_pause_sampling setting defer_trace(0) HOST %s, PID %lld, POSIX_TID %llu\n",
...
#ifndef NDEBUG
    if (getenv("OPENSS_DEBUG_COLLECTOR") != NULL) {
        fprintf(stderr,"offline_resume_sampling setting defer_trace(1) HOST %s, PID %lld, POSIX_TID %llu\n",
        tls->dso_header.host, tls->dso_header.pid, tls->dso_header.posix_tid);
    }
#endif
...
{
    OpenSS_SetSendToFile(&(tls->dso_header), "mpit", "openss-dsos");
    OpenSS_Send(&(tls->dso_header), (xdrproc_t)xdr_offline_data, &(tls->data));
    
    /* Send the offline "info" blob */
#ifndef NDEBUG
    if (getenv("OPENSS_DEBUG_COLLECTOR") != NULL) {
        fprintf(stderr,"offline_stop_sampling SENDS DSOS for HOST %s, PID %lld, POSIX_TID %llu\n",
        tls->dso_header.host, tls->dso_header.pid, tls->dso_header.posix_tid);
    }
#endif
#if 0
...
void offline_finish()
{
    ...
    OpenSS_SetSendToFile(&header, "mpit", "openss-info");
