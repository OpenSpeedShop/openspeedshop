...
/** @file
 *
 * Definition of the OmptPCollector class.
 *
 */
...
namespace {
    ...
    typedef std::map<StackTrace, OmptPDetail> SampleDetail;
 ...
 */
extern "C" CollectorImpl* omptp_LTX_CollectorFactory()
{
    return new OmptPCollector();
}
 ...
 * Default constructor.
 *
 * Constructs a new OmptP collector with the proper metadata.
 */
OmptPCollector::OmptPCollector() :
    CollectorImpl("omptp",
                  "OMPTP Profiling",
		  "")
{
    ...
    declareMetric(Metadata("inclusive_time", "Inclusive Time",
			   "Inclusive times in seconds.",
			   typeid(double)));
    declareMetric(Metadata("exclusive_time", "Exclusive Time",
			   "Exclusive times in seconds.",
			   typeid(double)));
    ...
    declareMetric(Metadata("min", "Minimum Time",
			   "Mininum times in seconds.",
			   typeid(double)));
    declareMetric(Metadata("max", "Maximum Time",
			   "Maximum times in seconds.",
			   typeid(double)));
    ...
    declareMetric(Metadata("stddev", "Standard Deviation Time",
			   "Standard Deviation times in seconds.",
			   typeid(double)));
 ...
 */
Blob OmptPCollector::getDefaultParameterValues() const
{
    // Setup an empty parameter structure    
    CBTF_ompt_parameters parameters;
    memset(&parameters, 0, sizeof(parameters));

    // Return the encoded blob to the caller
    return Blob(reinterpret_cast<xdrproc_t>(xdr_CBTF_ompt_parameters), &parameters);
}
 ...
 */
void OmptPCollector::getParameterValue(const std::string& parameter,
 ...
 */
void OmptPCollector::setParameterValue(const std::string& parameter,
 ...
 */
void OmptPCollector::startCollecting(const Collector& collector,
 ...
 */
void OmptPCollector::stopCollecting(const Collector& collector,
				 const ThreadGroup& threads) const
{
}
...
void OmptPCollector::getMetricValues(const std::string& metric,
				  ...
				  void* ptr) const
{
    ...
    blob.getXDRDecoding(reinterpret_cast<xdrproc_t>(xdr_CBTF_ompt_profile_data), &data);
...
//DEBUG
	...
	for(StackTrace::const_iterator
		...
		if(is_detail) {
			...
			.insert(
			    std::make_pair(trace, OmptPDetail())
			    ).first;
...
void OmptPCollector::getUniquePCValues( const Thread& thread,
                                     const Blob& blob,
