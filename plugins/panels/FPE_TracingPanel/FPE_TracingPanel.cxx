...
class EPOutputClass : public ss_ostream
{
  public:
    FPE_TracingPanel *ep;
    void setEP(FPE_TracingPanel *_ep) { ep = _ep;line_buffer = QString::null; };
    QString line_buffer;
  private:
 ...
 */
FPE_TracingPanel::FPE_TracingPanel(PanelContainer *pc, const char *n, ArgumentObject *ao) : Panel(pc, n)
{
...
// printf("A: command=(%s)\n", command.ascii() );
  ...
  QWidget *fpeTracingControlPanelContainerWidget =
    new QWidget( getBaseWidgetFrame(), "fpeTracingControlPanelContainerWidget" );
  topPC = createPanelContainer( fpeTracingControlPanelContainerWidget,
                              "PCSamplingControlPanel_topPC", NULL,
                              pc->getMasterPCList() );
  frameLayout->addWidget( fpeTracingControlPanelContainerWidget );

  OpenSpeedshop *mw = getPanelContainer()->getMainWindow();

  nprintf( DEBUG_PANELS ) ("Create a new fpeTracing experiment.\n");
...
// printf("C: command=(%s)\n", command.ascii() );
  ...
  fpeTracingControlPanelContainerWidget->show();
  topPC->show();
  topLevel = TRUE;
  topPC->topLevel = TRUE;
...
//    updateInitialStatus();
  ...
  {
    processLAO(ao->lao);
// printf("A: Attempt to remove the wizard panel from the fpele panel.\n");
    hideWizard();
  }
...
if( experiment != NULL )
{
  ...
  qaction = new QAction( this,  "FPE_TracingPanelSaveAs");
  qaction->addTo( contextMenu );
  qaction->setText( "Export Data..." );
  connect( qaction, SIGNAL( activated() ), this, SLOT( saveAsSelected() ) );
  qaction->setStatusTip( tr("Export data from all the FPE_TracingPanel's windows to an ascii file.") );
...
void
FPE_TracingPanel::compareExperimentsSelected()
{
  nprintf( DEBUG_PANELS ) ("FPE_TracingPanel::compareExperimentsSelected()\n");
...
FPE_TracingPanel::listener(void *msg)
{
  ...
  nprintf( DEBUG_MESSAGES ) ("FPE_TracingPanel::listener(%s) requested.\n", mo->msgType.ascii() );

  if( mo->msgType == getName() )
  {
    nprintf(DEBUG_MESSAGES) ("FPE_TracingPanel::listener() interested!\n");
...
// printf("B: Attempt to remove the wizard panel from the fpele panel.\n");
    loadManageProcessesPanel();
    QString name = QString("FPE Tracing Wizard");
...
FPE_TracingPanel::saveAsSelected()
{
  ...
  if( sfd == NULL )
  {
    ...
    sfd->setSelection(QString("FPE_TracingPanel.txt"));
    QString types(
                  "Any Files (*);;"
                  "Text files (*.txt);;"
                  );
...
void
FPE_TracingPanel::editPanelName()
{
...
void
FPE_TracingPanel::experimentStatus()
{
...
// printf("FPE_TracingPanel:: call (%s)'s listener routine.\n", statsPanel->getName());
    ExperimentObject *eo = Find_Experiment_Object((EXPID)expID);
    if( eo && eo->FW() )
    {
      experiment = eo->FW();
...
void
FPE_TracingPanel::loadManageProcessesPanel()
{
...
void
FPE_TracingPanel::wakeUpAndCheckExperimentStatus()
{
  printf("FPE_TracingPanel::wakeUpAndCheckExperimentStatus() entered\n");
}

void
FPE_TracingPanel::loadMain()
{
...
void
FPE_TracingPanel::statusUpdateTimerSlot()
{
//  statusTimer->stop();
  updateStatus();
}
...
void
FPE_TracingPanel::progressUpdate()
{
...
int
FPE_TracingPanel::processLAO(LoadAttachObject *lao)
{
...
void
FPE_TracingPanel::outputCLIData(QString *data)
{
// printf("data=%s\n", data->ascii() );
...
void
FPE_TracingPanel::attachProcessSelected()
{
...
void
FPE_TracingPanel::loadProgramSelected()
{
// printf("FPE_TracingPanel::loadProgramSelected()\n");
  mw->executableName = QString::null;
  ...
  {
// printf("FPE_TracingPanel::loadProgramSelected() executableName=%s\n", mw->executableName.ascii() );
    executableNameStr = mw->executableName;
    QString command =
