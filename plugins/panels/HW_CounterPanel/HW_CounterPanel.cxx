...
class EPOutputClass : public ss_ostream
{
  public:
    HW_CounterPanel *ep;
    void setEP(HW_CounterPanel *_ep) { ep = _ep;line_buffer = QString::null; };
    QString line_buffer;
  private:
 ...
 */
HW_CounterPanel::HW_CounterPanel(PanelContainer *pc, const char *n, ArgumentObject *ao) : Panel(pc, n)
{
...
if( attachFLAG )
{
  ...
  if( ao && ao->qstring_data )
  {
    ...
    {
      nprintf( DEBUG_PANELS ) ("we're coming in from the hwCounterWizardPanel.\n");
...
// printf("A: command=(%s)\n", command.ascii() );
  ...
  QWidget *hwCounterControlPanelContainerWidget =
    new QWidget( getBaseWidgetFrame(), "hwCounterControlPanelContainerWidget" );
  topPC = createPanelContainer( hwCounterControlPanelContainerWidget,
                              "PCSamplingControlPanel_topPC", NULL,
                              pc->getMasterPCList() );
  frameLayout->addWidget( hwCounterControlPanelContainerWidget );

  OpenSpeedshop *mw = getPanelContainer()->getMainWindow();

  nprintf( DEBUG_PANELS ) ("Create a new hwCounter experiment.\n");
...
// printf("C: command=(%s)\n", command.ascii() );
  ...
  hwCounterControlPanelContainerWidget->show();
  topPC->show();
  topLevel = TRUE;
  topPC->topLevel = TRUE;
...
{
  nprintf( DEBUG_PANELS ) ("HW_CounterPanel::menu() requested.\n");
...
if( experiment != NULL )
{
  ...
  qaction = new QAction( this,  "HW_CounterPanelSaveAs");
  qaction->addTo( contextMenu );
  qaction->setText( "Export Data..." );
  connect( qaction, SIGNAL( activated() ), this, SLOT( saveAsSelected() ) );
  qaction->setStatusTip( tr("Export data from all the HW_CounterPanel's windows to an ascii file.") );
...
{
  nprintf( DEBUG_PANELS ) ("HW_CounterPanel::save() requested.\n");
}
...
{
  nprintf( DEBUG_SAVEAS ) ("HW_CounterPanel::saveAs() requested.\n");
}
...
{
  nprintf( DEBUG_MESSAGES ) ("HW_CounterPanel::listener() requested.\n");
  ...
  nprintf( DEBUG_MESSAGES ) ("HW_CounterPanel::listener(%s) requested.\n", mo->msgType.ascii() );

  if( mo->msgType == getName() )
  {
    nprintf(DEBUG_MESSAGES) ("HW_CounterPanel::listener() interested!\n");
  ...
  {
    nprintf( DEBUG_MESSAGES ) ("HW_CounterPanel::listener() ClosingDownObject!\n");
...
// printf("B: Attempt to remove the wizard panel from the hwcle panel.\n");
    loadManageProcessesPanel();
...
void
HW_CounterPanel::updateInitialStatus()
{
// printf("A: loadMain()\n");
  loadMain();
}
...
void
HW_CounterPanel::saveAsSelected()
{
  ...
  if( sfd == NULL )
  {
    ...
    sfd->setSelection(QString("HW_CounterPanel.txt"));
    QString types(
                  "Any Files (*);;"
                  "Text files (*.txt);;"
                  );
...
void
HW_CounterPanel::loadSourcePanel()
{
nprintf( DEBUG_PANELS ) ("HW_CounterPanel::loadSourcePanel()\n");
//printf("HW_CounterPanel::loadSourcePanel()!!!!!\n");
...
//printf("try to find (%s)\n", name.ascii() );
  ...
  if( !sourcePanel )
  {
    PanelContainer *bestFitPC = getPanelContainer()->getMasterPC()->findBestFitPanelContainer(topPC);
    ArgumentObject *ao = new ArgumentObject("ArgumentObject", expID);
//printf("HW_CounterPanel::loadSourcePanel() create a new Source Panel!!\n");
...
void
HW_CounterPanel::experimentStatus()
{
...
Panel *
HW_CounterPanel::loadStatsPanel()
{
...
// printf("HW_CounterPanel:: call (%s)'s listener routine.\n", statsPanel->getName());
    ExperimentObject *eo = Find_Experiment_Object((EXPID)expID);
    if( eo && eo->FW() )
    {
      experiment = eo->FW();
...
void
HW_CounterPanel::loadManageProcessesPanel()
{
...
void
HW_CounterPanel::wakeUpAndCheckExperimentStatus()
{
  printf("HW_CounterPanel::wakeUpAndCheckExperimentStatus() entered\n");
}

void
HW_CounterPanel::loadMain()
{
...
// printf("hwc: eo->Determine_Status()=%d\n", eo->Determine_Status() );
    if( eo->Determine_Status() == ExpStatus_NonExistent || eo->Determine_Status() == ExpStatus_InError || eo->Determine_Status() == ExpStatus_Terminated )
    {
    statusLabelText->setText( tr(QString("Loaded saved data from file.") ) );
    runnableFLAG = FALSE;
...
void
HW_CounterPanel::statusUpdateTimerSlot()
{
//  statusTimer->stop();
  updateStatus();
}
...
void
HW_CounterPanel::progressUpdate()
{
...
void
HW_CounterPanel::outputCLIData(QString *data)
{
// printf("data=%s\n", data->ascii() );
