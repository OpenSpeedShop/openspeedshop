...
if( attachFLAG )
{
  ...
  if( ao && ao->qstring_data )
  {
    ...
    } else if( expIDString->toInt() > 0 )
    {
      ...
      // need to attach the usertimele collector.
...
// printf("A: command=(%s)\n", command.ascii() );
  ...
  QWidget *usertimeControlPanelContainerWidget =
    new QWidget( getBaseWidgetFrame(), "usertimeControlPanelContainerWidget" );
  topPC = createPanelContainer( usertimeControlPanelContainerWidget,
                              "PCSamplingControlPanel_topPC", NULL,
                              pc->getMasterPCList() );
  frameLayout->addWidget( usertimeControlPanelContainerWidget );

  OpenSpeedshop *mw = getPanelContainer()->getMainWindow();

  nprintf( DEBUG_PANELS ) ("Create a new usertime experiment.\n");
...
// printf("C: command=(%s)\n", command.ascii() );
  ...
  usertimeControlPanelContainerWidget->show();
  topPC->show();
  topLevel = TRUE;
  topPC->topLevel = TRUE;
...
//    updateInitialStatus();
  ...
  {
    processLAO(ao->lao);
// printf("A: Attempt to remove the wizard panel from the usertimele panel.\n");
    hideWizard();
  }
...
if( experiment != NULL )
{
  ...
  qaction = new QAction( this,  "UserTimePanelSaveAs");
  qaction->addTo( contextMenu );
  qaction->setText( "Export Data..." );
  connect( qaction, SIGNAL( activated() ), this, SLOT( saveAsSelected() ) );
  qaction->setStatusTip( tr("Export data from all the UserTimePanel's windows to an ascii file.") );
...
void
UserTimePanel::compareExperimentsSelected()
{
  nprintf( DEBUG_PANELS ) ("UserTimePanel::compareExperimentsSelected()\n");
...
UserTimePanel::save()
{
  nprintf( DEBUG_PANELS ) ("UserTimePanel::save() requested.\n");
}
...
{
  nprintf( DEBUG_SAVEAS ) ("UserTimePanel::saveAs() requested.\n");
}
...
{
  nprintf( DEBUG_MESSAGES ) ("UserTimePanel::listener() requested.\n");
  ...
  nprintf( DEBUG_MESSAGES ) ("UserTimePanel::listener(%s) requested.\n", mo->msgType.ascii() );

  if( mo->msgType == getName() )
  {
    nprintf(DEBUG_MESSAGES) ("UserTimePanel::listener() interested!\n");
  ...
  {
    nprintf( DEBUG_MESSAGES ) ("UserTimePanel::listener() ClosingDownObject!\n");
...
// printf("B: Attempt to remove the wizard panel from the usertimele panel.\n");
    loadManageProcessesPanel();
    QString name = QString("User Time Wizard");
...
void
UserTimePanel::updateInitialStatus()
{
// printf("A: loadMain()\n");
  loadMain();
}
...
UserTimePanel::saveAsSelected()
{
  ...
  if( sfd == NULL )
  {
    ...
    sfd->setSelection(QString("UserTimePanel.txt"));
    QString types(
                  "Any Files (*);;"
                  "Text files (*.txt);;"
                  );
...
{
nprintf( DEBUG_PANELS ) ("UserTimePanel::loadSourcePanel()\n");
//printf("UserTimePanel::loadSourcePanel()!!!!!\n");
...
//printf("try to find (%s)\n", name.ascii() );
  ...
  if( !sourcePanel )
  {
    PanelContainer *bestFitPC = getPanelContainer()->getMasterPC()->findBestFitPanelContainer(topPC);
    ArgumentObject *ao = new ArgumentObject("ArgumentObject", expID);
//printf("UserTimePanel::loadSourcePanel() create a new Source Panel!!\n");
...
void
UserTimePanel::experimentStatus()
{
...
void
UserTimePanel::loadManageProcessesPanel()
{
...
void
UserTimePanel::wakeUpAndCheckExperimentStatus()
{
  printf("UserTimePanel::wakeUpAndCheckExperimentStatus() entered\n");
}

void
UserTimePanel::loadMain()
{
...
//printf("UserTimePanel:A: create a source panel.\n");
...
// printf("usertime: eo->Determine_Status()=%d\n", eo->Determine_Status() );
    if( eo->Determine_Status() == ExpStatus_NonExistent || eo->Determine_Status() == ExpStatus_InError || eo->Determine_Status() == ExpStatus_Terminated )
    {
    statusLabelText->setText( tr(QString("Loaded saved data from file.") ) );
    runnableFLAG = FALSE;
...
void
UserTimePanel::statusUpdateTimerSlot()
{
//  statusTimer->stop();
  updateStatus();
}
...
void
UserTimePanel::attachProcessSelected()
{
...
void
UserTimePanel::loadProgramSelected()
{
// printf("UserTimePanel::loadProgramSelected()\n");
  mw->executableName = QString::null;
  ...
  {
// printf("UserTimePanel::loadProgramSelected() executableName=%s\n", mw->executableName.ascii() );
    executableNameStr = mw->executableName;
    QString command =
