...
class EPOutputClass : public ss_ostream
{
  public:
    MPIPanel *ep;
    void setEP(MPIPanel *_ep) { ep = _ep;line_buffer = QString::null; };
    QString line_buffer;
  private:
 ...
 */
MPIPanel::MPIPanel(PanelContainer *pc, const char *n, ArgumentObject *ao) : Panel(pc, n)
{
...
if( attachFLAG )
{
  ...
  if( ao && ao->qstring_data )
  {
    ...
    {
      nprintf( DEBUG_PANELS ) ("we're coming in from the mpiWizardPanel.\n");
    } else if( expIDString->toInt() > 0 )
    {
      ...
      // need to attach the mpile collector.
...
//      Thread t1 = *ti; 
      ...
      if( cgrp.size() == 0 )
      {
        ...
        QString command = QString("expAttach -x %1 mpi").arg(expID);
...
// printf("A: command=(%s)\n", command.ascii() );
  ...
  QWidget *mpiControlPanelContainerWidget =
    new QWidget( getBaseWidgetFrame(), "mpiControlPanelContainerWidget" );
  ...
  nprintf( DEBUG_PANELS ) ("Create a new mpi experiment.\n");

  CLIInterface *cli = getPanelContainer()->getMainWindow()->cli;

  if( expID == -1 )
  {
    // We're coming in cold, or we're coming in from the mpiWizardPanel.
    QString command = QString::null;
    command = QString("expCreate mpi\n");
...
if( experiment != NULL )
{
  ...
  qaction = new QAction( this,  "MPIPanelSaveAs");
  qaction->addTo( contextMenu );
  qaction->setText( "Export Data..." );
  connect( qaction, SIGNAL( activated() ), this, SLOT( saveAsSelected() ) );
  qaction->setStatusTip( tr("Export data from all the MPIPanel's windows to an ascii file.") );
...
void
MPIPanel::compareExperimentsSelected()
{
  nprintf( DEBUG_PANELS ) ("MPIPanel::compareExperimentsSelected()\n");
...
MPIPanel::listener(void *msg)
{
  ...
  nprintf( DEBUG_MESSAGES ) ("MPIPanel::listener(%s) requested.\n", mo->msgType.ascii() );

  if( mo->msgType == getName() )
  {
    nprintf(DEBUG_MESSAGES) ("MPIPanel::listener() interested!\n");
...
// printf("B: Attempt to remove the wizard panel from the mpile panel.\n");
    loadManageProcessesPanel();
...
MPIPanel::saveAsSelected()
{
  ...
  if( sfd == NULL )
  {
    ...
    sfd->setSelection(QString("MPIPanel.txt"));
    QString types(
                  "Any Files (*);;"
                  "Text files (*.txt);;"
                  );
...
{
nprintf( DEBUG_PANELS ) ("MPIPanel::loadSourcePanel()\n");
//printf("MPIPanel::loadSourcePanel()!!!!!\n");
...
//printf("try to find (%s)\n", name.ascii() );
  ...
  if( !sourcePanel )
  {
    PanelContainer *bestFitPC = getPanelContainer()->getMasterPC()->findBestFitPanelContainer(topPC);
    ArgumentObject *ao = new ArgumentObject("ArgumentObject", expID);
//printf("MPIPanel::loadSourcePanel() create a new Source Panel!!\n");
...
// printf("MPIPanel:: call (%s)'s listener routine.\n", statsPanel->getName());
    ExperimentObject *eo = Find_Experiment_Object((EXPID)expID);
    if( eo && eo->FW() )
    {
      experiment = eo->FW();
...
void
MPIPanel::loadManageProcessesPanel()
{
...
void
MPIPanel::wakeUpAndCheckExperimentStatus()
{
  printf("MPIPanel::wakeUpAndCheckExperimentStatus() entered\n");
}
...
//printf("MPIPanel:A: create a source panel.\n");
...
// printf("mpi: eo->Determine_Status()=%d\n", eo->Determine_Status() );
    if( eo->Determine_Status() == ExpStatus_NonExistent || eo->Determine_Status() == ExpStatus_InError || eo->Determine_Status() == ExpStatus_Terminated )
    {
    statusLabelText->setText( tr(QString("Loaded saved data from file.") ) );
    runnableFLAG = FALSE;
...
void
MPIPanel::statusUpdateTimerSlot()
{
//  statusTimer->stop();
  updateStatus();
}
...
int
MPIPanel::processLAO(LoadAttachObject *lao)
{
...
void
MPIPanel::outputCLIData(QString *data)
{
// printf("data=%s\n", data->ascii() );
