...
// These are needed to manage mpip collector data.
...
#define set_MPIP_values(value_array, sort_extime)                                       \
...
#define def_Detail_values def_MPIP_values
#define set_Detail_values set_MPIP_values
...
static std::string allowed_mpip_V_options[] = {
  "LinkedObject",
  "LinkedObjects",
  "Dso",
  "Dsos",
...
static bool define_mpip_columns (
            CommandObject *cmd,
...
#include "SS_View_metric_expressions.hxx"
      ...
      if (m_range->is_range) {
        ...
        if (!strcasecmp(M_Name.c_str(), "mpip")) {
         // We only know what to do with the mpip collector.
          std::string s("The specified collector, " + C_Name +
                        ", can not be displayed as part of a 'mpip' view.");
...
static bool mpip_definition (
    ...
    bool all_valid = Validate_V_Options (cmd, allowed_mpip_V_options);
    if ( all_valid == false ) {
      return false;
    }

    CV.push_back (Get_Collector (exp->FW(), "mpip"));  // Define the collector
    ...
    return define_mpip_columns (cmd, exp, CV, MV, IV, HV, vfc);
}

static std::string VIEW_mpip_brief = "MPI Profiling (MPIP) Report";
static std::string VIEW_mpip_short = "Report the amount of time spent in a MPI calls.";
static std::string VIEW_mpip_long  =
                  "\nA positive integer can be added to the end of the keyword"
                  " 'mpip' to indicate the maximum number of items in the report."
                  ...
                  " \n\t'-m exclusive_time' reports the wall clock time used in the MPI function."
                  " \n\t'-m inclusive_time' reports the wall clock time used in the aggregate"
                  " by the unit and all the units it calls."
                  " \n\t'-m percent' reports the percent of total cpu the MPI function represents."
                  " \n\t'-m count' reports the number calls into the MPI function."
// Get the description of the BY-Thread metrics.
#include "SS_View_bythread_help.hxx"
                  "\n";
static std::string VIEW_mpip_example = "\texpView mpip\n"
                                           "\texpView -v LinkedObjects mpip\n"
                                           "\texpView -v Statements mpip20\n"
                                           "\texpView -v Functions mpip10 -m mpip::inclusive_time\n"
                                           "\texpView mpip20 -m inclusive_time, exclusive_time\n"
                                           "\texpView -v CallTrees,FullStack mpip10 -m count\n";
static std::string VIEW_mpip_metrics[] =
  { "exclusive_time",
    "exclusive_details",
    "inclusive_time",
    "inclusive_details",
...
static std::string VIEW_mpip_collectors[] =
  { "mpip",
    ""
  };
class mpip_view : public ViewType {

 public: 
  mpip_view() : ViewType ("mpip",
                         VIEW_mpip_brief,
                         VIEW_mpip_short,
                         VIEW_mpip_long,
                         VIEW_mpip_example,
                        &VIEW_mpip_metrics[0],
                        &VIEW_mpip_collectors[0],
                         true) {
  }
  virtual bool GenerateView (CommandObject *cmd, ExperimentObject *exp, int64_t topn,
    ...
    if (mpip_definition (cmd, exp, topn, tgrp, CV, MV, IV, HV, vfc)) {
    ...
    Mark_Cmd_With_Soft_Error(cmd, "(We could not determine what information to report for 'mpip' view.)");
    return false;
  }
};
...
extern "C" void mpip_view_LTX_ViewFactory () {
  Define_New_View (new mpip_view());
}
