...
// These are needed to manage iop collector data.
...
static std::string allowed_iop_V_options[] = {
  "LinkedObject",
  "LinkedObjects",
  "Dso",
  "Dsos",
...
static bool define_iop_columns (
            CommandObject *cmd,
...
#include "SS_View_metric_expressions.hxx"
      ...
      if (m_range->is_range) {
        ...
        if (!strcasecmp(M_Name.c_str(), "iop")) {
         // We only know what to do with the iop collector.
          std::string s("The specified collector, " + C_Name +
                        ", can not be displayed as part of a 'iop' view.");
...
static bool iop_definition (
    ...
    bool all_valid = Validate_V_Options (cmd, allowed_iop_V_options);
    if ( all_valid == false ) {
      return false;
    }


    CV.push_back (Get_Collector (exp->FW(), "iop"));  // Define the collector
...
#include "SS_View_bythread_help.hxx"
                  "\n";
static std::string VIEW_iop_example = "\texpView iop\n"
                                           "\texpView -v LinkedObjects iop\n"
                                           "\texpView -v Statements iop20\n"
                                           "\texpView -v Functions iop10 -m iop::inclusive_time\n"
                                           "\texpView iop20 -m inclusive_time, exclusive_time\n"
                                           "\texpView -v CallTrees,FullStack iop10 -m count\n";
static std::string VIEW_iop_metrics[] =
  { "exclusive_time",
    "exclusive_details",
    "inclusive_time",
    "inclusive_details",
...
static std::string VIEW_iop_collectors[] =
  { "iop",
    ""
  };
class iop_view : public ViewType {

 public: 
  iop_view() : ViewType ("iop",
                         VIEW_iop_brief,
                         VIEW_iop_short,
                         VIEW_iop_long,
                         VIEW_iop_example,
                        &VIEW_iop_metrics[0],
                        &VIEW_iop_collectors[0],
                         true) {
  }
  virtual bool GenerateView (CommandObject *cmd, ExperimentObject *exp, int64_t topn,
    ...
    if (iop_definition (cmd, exp, topn, tgrp, CV, MV, IV, HV, vfc)) {
    ...
    Mark_Cmd_With_Soft_Error(cmd, "(We could not determine what information to report for 'iop' view.)");
    return false;
  }
};
...
extern "C" void iop_view_LTX_ViewFactory () {
  Define_New_View (new iop_view());
}
