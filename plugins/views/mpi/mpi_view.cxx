  ...
  if (p_tlist->begin() == p_tlist->end()) {
   ...
   // There is a list.  Is there a "-f" specifier?
    ...
    if ((f_list == NULL) || (f_list->empty()) ||
     ...
     // There is no Function filtering requested or a ButterFly views is requested.
     // Get all the functions in the already selected thread groups.
     // Function filtering will be done later for ButterFly views.
...
static bool MPI_Trace_Report(
 ...
 // Get the list of desired functions.
  ...
  if (objects.empty()) {
    Mark_Cmd_With_Soft_Error(cmd, "(There are no functions found for the 'MPI -v Trace' report.)");
    return false;
  }
  ...
  try {
   ...
   // Acquire base set of metric values.
    ...
    if (raw_items->begin() == raw_items->end()) {
      Mark_Cmd_With_Soft_Error(cmd, "(There are no data samples available for the requested MPI functions.)");
      return false;
    }
    ...
    for (fi = raw_items->begin(); fi != raw_items->end(); fi++) {
     // Foreach MPI function ...
      ...
      for (si = (*fi).second.begin(); si != (*fi).second.end(); si++) {
        ...
        std::vector<MPIDetail> details = (*si).second;
...
static bool MPI_Function_Report(
 ...
 // Get the list of desired functions.
  ...
  Determine_Objects ( cmd, exp, tgrp, objects);
  if (objects.empty()) {
    Mark_Cmd_With_Soft_Error(cmd, "(There are no objects specified for the basic MPI report.)");
    return false;
  }
...
static bool MPI_CallStack_Report (
 ...
 // Get the list of desired functions.
  ...
  Determine_Objects ( cmd, exp, tgrp, objects);
  if (objects.empty()) {
    Mark_Cmd_With_Soft_Error(cmd, "(There are no functions found for the 'MPI -v CallStack' report.)");
    return false;
  }
    ...
    for (fi = raw_items->begin(); fi != raw_items->end(); fi++) {
     // Foreach MPI function ...
      ...
      for (sti = (*fi).second.begin(); sti != (*fi).second.end(); sti++) {
        ...
        std::vector<MPIDetail> details = (*sti).second;
...
static bool MPI_ButterFly_Report (
              CommandObject *cmd, ExperimentObject *exp, int64_t topn,
              ThreadGroup& tgrp, std::vector<Collector>& CV, std::vector<std::string>& MV,
              std::vector<ViewInstruction *>& IV, std::vector<std::string>& HV,
              std::list<CommandResult *>& view_output) {
 ...
 // Get the list of desired functions.
  ...
  if (objects.empty()) {
    Mark_Cmd_With_Soft_Error(cmd, "(There are no functions found for the 'MPI -v ButterFly' report.)");
    return false;
  }
  ...
  try {
   ...
   // Generate a separate butterfly view for each function in the list.
    std::map<Address, CommandResult *> knownTraces;
    std::map<Function, std::map<Framework::StackTrace, std::vector<MPIDetail> > >::iterator fi1;
    for (fi1 = raw_items->begin(); fi1 != raw_items->end(); fi1++) {
     ...
     // Capture each StackTrace in the inclusive_detail list.
      std::set<Framework::StackTrace, ltST> StackTraces_Processed;
      std::map<Framework::StackTrace, std::vector<MPIDetail> >:: iterator si1;
      for (si1 = (*fi1).second.begin(); si1 != (*fi1).second.end(); si1++) {
        Framework::StackTrace st = (*si1).first;
        std::vector<MPIDetail> details = (*si1).second;
      ...
      switch (vfc) {
       case VFC_Trace:
        if (Look_For_KeyWord(cmd, "ButterFly")) {
          return MPI_ButterFly_Report (cmd, exp, topn, tgrp, CV, MV, IV, HV, view_output);
        } else {
