...
#define set_OmptP_values(value_array, sort_extime)                                       \
...
#define def_Detail_values def_OmptP_values
#define set_Detail_values set_OmptP_values
...
static std::string allowed_omptp_V_options[] = {
  "LinkedObject",
  "LinkedObjects",
  "Dso",
  "Dsos",
...
static bool define_omptp_columns (
            CommandObject *cmd,
...
#include "SS_View_metric_expressions.hxx"
      ...
      if (m_range->is_range) {
        ...
        if (!strcasecmp(M_Name.c_str(), "omptp")) {
         // We only know what to do with the omptp collector.
          std::string s("The specified collector, " + C_Name +
                        ", can not be displayed as part of a 'omptp' view.");
...
static bool omptp_definition (
    ...
    bool all_valid = Validate_V_Options (cmd, allowed_omptp_V_options);
    if ( all_valid == false ) {
      return false;
    }


    CV.push_back (Get_Collector (exp->FW(), "omptp"));  // Define the collector
    ...
    return define_omptp_columns (cmd, exp, CV, MV, IV, HV, vfc);
}

static std::string VIEW_omptp_brief = "OMPT Profiling (OmptP) Report";
static std::string VIEW_omptp_short = "Report profile of OMPT callbacks.";
static std::string VIEW_omptp_long  =
                  "\nA positive integer can be added to the end of the keyword"
                  " 'omptp' to indicate the maximum number of items in the report."
...
#include "SS_View_bythread_help.hxx"
                  "\n";
static std::string VIEW_omptp_example = "\texpView omptp\n"
                                           "\texpView -v LinkedObjects omptp\n"
                                           "\texpView -v Statements omptp20\n"
                                           "\texpView -v Functions omptp10 -m omptp::inclusive_time\n"
                                           "\texpView omptp20 -m inclusive_time, exclusive_time\n"
                                           "\texpView -v CallTrees,FullStack omptp10 -m count\n";
static std::string VIEW_omptp_metrics[] =
  { "exclusive_time",
    "exclusive_details",
    "inclusive_time",
    "inclusive_details",
...
static std::string VIEW_omptp_collectors[] =
  { "omptp",
    ""
  };
class omptp_view : public ViewType {

 public: 
  omptp_view() : ViewType ("omptp",
                         VIEW_omptp_brief,
                         VIEW_omptp_short,
                         VIEW_omptp_long,
                         VIEW_omptp_example,
                        &VIEW_omptp_metrics[0],
                        &VIEW_omptp_collectors[0],
                         true) {
  }
  virtual bool GenerateView (CommandObject *cmd, ExperimentObject *exp, int64_t topn,
    ...
    if (omptp_definition (cmd, exp, topn, tgrp, CV, MV, IV, HV, vfc)) {
    ...
    Mark_Cmd_With_Soft_Error(cmd, "(We could not determine what information to report for 'omptp' view.)");
    return false;
  }
};
...
extern "C" void omptp_view_LTX_ViewFactory () {
  Define_New_View (new omptp_view());
}
