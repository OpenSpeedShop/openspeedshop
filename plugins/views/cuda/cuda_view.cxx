...
#define get_CUDA_invalues(primary,num_calls)                      \
...
{           int64_t len = stdv.size();                  \
            for (int64_t i = 0; i < len; i++) {         \
             /* Use macro to combine all the values. */ \
              get_CUDA_invalues(stdv[i],num_calls)       \
            }                                           \
}
...
static std::string allowed_cuda_V_options[] = {
  "Function",
  "Functions",
  "Trace",
  "ButterFly",
...
static bool define_cuda_columns (
            CommandObject *cmd,
            ExperimentObject *exp,
...
static bool cuda_definition ( CommandObject *cmd, ExperimentObject *exp, int64_t topn,
    ...
    bool all_valid = Validate_V_Options (cmd, allowed_cuda_V_options);
    if ( all_valid == false ) {
      return false;
    }
...
class cuda_view : public ViewType {

 public: 
  cuda_view() : ViewType ("cuda",
                         VIEW_cuda_brief,
                         VIEW_cuda_short,
                         VIEW_cuda_long,
                         VIEW_cuda_example,
                        &VIEW_cuda_metrics[0],
                        &VIEW_cuda_collectors[0],
                         true) {
  }
  virtual bool GenerateView (CommandObject *cmd, ExperimentObject *exp, int64_t topn,
    ...
    Mark_Cmd_With_Soft_Error(cmd, "(We could not determine what information to report for 'cuda' view.)");
    return false;
  }
};
