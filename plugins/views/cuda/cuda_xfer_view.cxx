...
static const std::string kOptions[] = {
    "Xfer", // This is the option that selects this particular sub-view
    "ButterFly",
    "CallTree", "CallTrees",
    "DontExpand",
    "Dso", "Dsos",
...
// These are needed to manage CUDA data transfer data
...
#define size_temp VMulti_free_temp+9
#define kind_temp VMulti_free_temp+10
#define src_temp VMulti_free_temp+11
#define dest_temp VMulti_free_temp+12
#define async_temp VMulti_free_temp+13
...
#define def_CUDA_values               \
    ...
    std::string detail_kind = "";     \
    std::string detail_src = "";      \
    std::string detail_dest = "";     \
    std::string detail_async = "";
...
#define set_CUDA_values(value_array, sort_extime)                         \
    ...
    if (num_temps > size_temp)                                            \
    {                                                                     \
        CommandResult* p = CRPTR(detail_size);                            \
        value_array[size_temp] = p;                                       \
    }                                                                     \
    if (num_temps > kind_temp)                                            \
    {                                                                     \
        CommandResult* p = CRPTR(detail_kind);                            \
        p->SetValueIsID();                                                \
        value_array[kind_temp] = p;                                       \
    }                                                                     \
    if (num_temps > src_temp)                                             \
    {                                                                     \
        CommandResult* p = CRPTR(detail_src);                             \
        p->SetValueIsID();                                                \
        value_array[src_temp] = p;                                        \
    }                                                                     \
    if (num_temps > dest_temp)                                            \
    {                                                                     \
        CommandResult* p = CRPTR(detail_dest);                            \
        p->SetValueIsID();                                                \
        value_array[dest_temp] = p;                                       \
    }                                                                     \
    if (num_temps > async_temp)                                           \
    {                                                                     \
        CommandResult* p = CRPTR(detail_async);                           \
        value_array[async_temp] = p;                                      \
    }
                                ...
                                View_Form_Category vfc)
{
    ...
    if (vfc == VFC_Trace)
    {
        ...
        MetricMap["kind"] = kind_temp;
        MetricMap["src"] = src_temp;
        MetricMap["dest"] = dest_temp;
        MetricMap["async"] = async_temp;
    }
        ...
        for (mi = p_slist->begin(); mi != p_slist->end(); mi++)
        {
            ...
            if (M_Name.length() > 0)
            {
                ...
                else if (!strcasecmp(M_Name.c_str(), "size"))
                {
                    if (vfc == VFC_Trace)
                    {
                        PUSH_IV(VIEWINST_Display_Tmp, last_column++, size_temp);
                        PUSH_HV("Size");
                    }
                    else
                    {
                        WARN("Warning: '-m size' only "
                             "supported for '-v Trace' option.");
                    }
                }
                else if (!strcasecmp(M_Name.c_str(), "kind"))
                {
                    if (vfc == VFC_Trace)
                    {
                        PUSH_IV(VIEWINST_Display_Tmp, last_column++, kind_temp);
                        PUSH_HV("Kind");
                    }
                    else
                    {
                        WARN("Warning: '-m kind' only "
                             "supported for '-v Trace' option.");
                    }
                }
                else if (!strcasecmp(M_Name.c_str(), "src"))
                {
                    if (vfc == VFC_Trace)
                    {
                        PUSH_IV(VIEWINST_Display_Tmp, last_column++, src_temp);
                        PUSH_HV("Source");
                    }
                    else
                    {
                        WARN("Warning: '-m src' only "
                             "supported for '-v Trace' option.");
                    }
                }
                else if (!strcasecmp(M_Name.c_str(), "dest"))
                {
                    if (vfc == VFC_Trace)
                    {
                        PUSH_IV(VIEWINST_Display_Tmp, last_column++, dest_temp);
                        PUSH_HV("Destination");
                    }
                    else
                    {
                        WARN("Warning: '-m dest' only "
                             "supported for '-v Trace' option.");
                    }
                }
                else if (!strcasecmp(M_Name.c_str(), "async"))
                {
                    if (vfc == VFC_Trace)
                    {
                        PUSH_IV(VIEWINST_Display_Tmp, last_column++,
    ...
    if (cuda_definition(cmd, exp, topn, tgrp, CV, MV, IV, HV, vfc))
    {
        ...
        case VFC_CallStack:
            if (Look_For_KeyWord(cmd, "ButterFly"))
            {
                return Detail_ButterFly_Report(
                    ...
                    reinterpret_cast<std::vector<CUDAXferDetail>*>(NULL),
                    view_output
                    );
            }
            else
        ...
        case VFC_Trace:
            return Detail_Trace_Report(
                cmd, exp, topn, tgrp, CV, MV, IV, HV,
                Determine_Metric_Ordering(IV),
                reinterpret_cast<CUDAXferDetail*>(NULL), view_output
                );
